...
function NavigationMenu({
  ...
  return (
    <NavigationMenuPrimitive.Root
      ...
      className={cn(
        "group/navigation-menu relative flex max-w-max flex-1 items-center justify-center uppercase ",
        className,
      )}
      {...props}
    >
...
function NavigationMenuTrigger({
  ...
  return (
    ...
    >
      {children}{" "}
      <ChevronDownIcon
        className="relative top-px ml-2 size-3 transition duration-300 group-data-[state=open]:rotate-180 "
        aria-hidden="true"
      />
    </NavigationMenuPrimitive.Trigger>
  );
  ...
  return (
    <div className={cn("absolute top-full   z-50 flex justify-center ")}>
      <NavigationMenuPrimitive.Viewport
        ...
        className={cn(
          "ui-floating origin-top-center text-popover-foreground data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-90 relative mt-1.5 h-(--radix-navigation-menu-viewport-height) overflow-hidden rounded-xl md:w-(--radix-navigation-menu-viewport-width)",
          className,
        )}
        {...props}
      />
